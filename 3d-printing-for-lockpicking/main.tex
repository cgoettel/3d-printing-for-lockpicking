...
The rise of cryptographically-keyed, electronically-controlled physical locking systems provides an alternative that avoids many of the vulnerabilities presented here. Increasing adoption of these systems continues to drive down their cost and increases 
% What do you mean by "visibility" here?
as well as increase visibility into the secure operation of these systems.
% What is meant by "comprising"? The parts that make up the system? Or are we trying to say the parts of the system that can be compromised?
As with any access control system, the responsibility falls on the adopting party to ensure that the system is sound both in terms of overall, secure implementation and the system's comprising parts. This includes finding alternatives to preset keys (e.g., smart cards and smart card readers), the supporting server infrastructure, and the relating
% related?
locking mechanisms themselves (e.g., magnetic or electromechanical locks).
% Can we change "preset keys" to "standard/ordinary keys"? I think the vocab would make more sense to a more general audience. Something closer to "brick and mortar" would help clarify the idea better, eh?
